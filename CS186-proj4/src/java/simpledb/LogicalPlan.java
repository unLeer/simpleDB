    ...
    public DbIterator physicalPlan(TransactionId t, Map<String,TableStats> baseTableStats, boolean explain) throws ParsingException {
        ...
        while (filterIt.hasNext()) {
            ...
            try {//td.fieldNameToIndex(disambiguateName(lf.fieldPureName))
                ftyp = td.getFieldType(td.fieldNameToIndex(lf.fieldPureName));
            } catch (java.util.NoSuchElementException e) {
                throw new ParsingException("Unknown field in filter expression " + lf.fieldPureName);
            }
            ...
            Predicate p = null;
            try {
                p = new Predicate(subplan.getTupleDesc().fieldNameToIndex(lf.fieldPureName), lf.p,f);
            } catch (NoSuchElementException e) {
                throw new ParsingException("Unknown field " + lf.fieldPureName);
            }
            ...
            double sel= s.estimateSelectivity(subplan.getTupleDesc().fieldNameToIndex(lf.fieldPureName), lf.p, f);
...
//                int  id;
                String[] fields = si.fname.split("\\.");
                try {
//                    id = 
                    //td.fieldNameToIndex(si.fname);
                    td.fieldNameToIndex(fields[1]);
                } catch (NoSuchElementException e) {
                ...
                outTypes.add(Type.INT_TYPE);  //the type of all aggregate functions is INT
                    ...
                    int id;
                    try {
                        id = td.fieldNameToIndex(fields[1]);
                    } catch (NoSuchElementException e) {
                        throw new ParsingException("Unknown field " +  si.fname + " in SELECT list in"+td);
                    }
                    outFields.add(id);
                    outTypes.add(td.getFieldType(id));
